...
MSG_ERROR db 13,10, "ERROR!!! AX HAVE CODE!$"
MSG_ALL_OKEY db 13,10 , "Memory asking is normal$"
MSG_FREE_MEMORY db 13,10, "Free memory done.$"

MSG_ENTER db 13,10, "-------------------------------$"
       

;Num    PSP_address    Size(b)    Name
;ПРОЦЕДУРЫ
;-------------------------------
...
FREE_MEMORY PROC near
	...
	mov ah,4Ah
	int 21h
	mov dx, offset MSG_FREE_MEMORY
	call WRITE_STR
	ret
FREE_MEMORY ENDP


ASK_MEMORY PROC near
    clc				  ; сброс флага
	mov bx,1000h      ;запрос 4096 параграф
	mov ah, 48h
	int 21h
	jc ask_err       ; флаг CF установлен
	mov dx, offset MSG_ALL_OKEY
	jmp end_ask
ask_err: 
